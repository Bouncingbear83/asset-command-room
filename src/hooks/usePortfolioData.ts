...
const KNOWN_COLS = [
  ...
  "cumulative_twr_total", "jb_mv", "ab_mv", "eb_mv", "cash_jb", "cash_ab",
  "cash_eb", "total_jb", "total_ab", "total_eb", "deposits_in_period_jb",
  "deposits_in_period_ab", "deposits_in_period_eb", "sub_period_rtn_jb",
  "sub_period_rtn_ab", "sub_period_rtn_eb", "cumulative_twr_jb",
  "cumulative_twr_ab", "cumulative_twr_eb",
  "note", "trigger_type", "trigger_price_add", "trigger_price_exit",
...
function parsePerformance(rows: Record<string, any>[]) {
  return rows
    ...
    .map((row) => ({
      ...
      // JISA columns
      jbMv: parseMv(findCol(row, "jb_mv", "JB_MV")),
      abMv: parseMv(findCol(row, "ab_mv", "AB_MV")),
      ebMv: parseMv(findCol(row, "eb_mv", "EB_MV")),
      cashJb: parseMv(findCol(row, "cash_jb", "CASH_JB")),
      cashAb: parseMv(findCol(row, "cash_ab", "CASH_AB")),
      cashEb: parseMv(findCol(row, "cash_eb", "CASH_EB")),
      totalJb: parseMv(findCol(row, "total_jb", "TOTAL_JB")),
      totalAb: parseMv(findCol(row, "total_ab", "TOTAL_AB")),
      totalEb: parseMv(findCol(row, "total_eb", "TOTAL_EB")),
      depositsJb: parseMv(findCol(row, "deposits_in_period_jb", "DEPOSITS_IN_PERIOD_JB")),
      depositsAb: parseMv(findCol(row, "deposits_in_period_ab", "DEPOSITS_IN_PERIOD_AB")),
      depositsEb: parseMv(findCol(row, "deposits_in_period_eb", "DEPOSITS_IN_PERIOD_EB")),
      subPeriodRtnJb: parsePercentLike(findCol(row, "sub_period_rtn_jb", "SUB_PERIOD_RTN_JB")),
      subPeriodRtnAb: parsePercentLike(findCol(row, "sub_period_rtn_ab", "SUB_PERIOD_RTN_AB")),
      subPeriodRtnEb: parsePercentLike(findCol(row, "sub_period_rtn_eb", "SUB_PERIOD_RTN_EB")),
      cumulativeTwrJb: parseFractionToPercent(findCol(row, "cumulative_twr_jb", "CUMULATIVE_TWR_JB")),
      cumulativeTwrAb: parseFractionToPercent(findCol(row, "cumulative_twr_ab", "CUMULATIVE_TWR_AB")),
      cumulativeTwrEb: parseFractionToPercent(findCol(row, "cumulative_twr_eb", "CUMULATIVE_TWR_EB")),
    }));
}
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setState({
        ...
        jisaHoldings: (() => { const r = parseJisaHoldings(jisaHoldingsRaw); return r.holdings; })(),
        jisaTotals: parseJisaHoldings(jisaHoldingsRaw).totals,
        cashSipp,
        cashIsa,
        cashTotal,
