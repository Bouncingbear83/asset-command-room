import { LiveLayer, LiveNarrative, LiveWatchItem } from "@/hooks/usePortfolioData";
...
export default function LayersTab({ liveData, watchlist, narrative }: Props) {
  ...
  const layerNarrative = narrative.layer_narrative || narrative.layer_view || narrative.layer_commentary || "";
    ...
    <div>
      <div style={card}>
        <div style={cardHeader}>
          ...
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: liveData.length > 0 ? "var(--green)" : "var(--text-dim)" }}>
            {liveData.length > 0 ? "● LIVE" : "● NO DATA"}
          </span>
        </div>
            ...
            return (
              ...
              >
                <div>
                  <div
                    style={{
                      ...
                      textTransform: "uppercase",
                      letterSpacing: "0.1em",
                      color: layer.hexColor || "var(--text-mid)",
                    }}
                  >
                    {layer.name}
                  </div>
                  ...
                  {isCash && (
                    <div
                      style={{
                        ...
                        width: `${Math.min(layer.current * 5, 100)}%`,
                        maxWidth: "100%",
                        opacity: 0.4,
                        transition: "width 0.8s ease",
                      }}
                ...
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--text)", textAlign: "right" }}>
                  {layer.current.toFixed(1)}%
                </div>
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--text-dim)" }}>
                  {isCash ? "" : `/${layer.target}%`}
                </div>
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: diffColor, textAlign: "right", fontWeight: 600 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <div style={card}>
          <div style={cardHeader}>
            ...
            {liveGapLayers.length > 0 && <span style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--green)" }}>● LIVE</span>}
          </div>
          <div style={{ padding: "0 20px 12px" }}>
            {layerNarrative && (
              <div style={{ ...emptyState, paddingBottom: 12, borderBottom: "1px solid rgba(28,28,48,0.4)" }}>
                {layerNarrative}
              </div>
            )}
            ...
            {liveGapLayers.map((layer) => (
              <div key={layer.name} style={{ padding: "12px 0", borderBottom: "1px solid rgba(28,28,48,0.4)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                  <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, fontWeight: 700, color: layer.hexColor || "var(--text)" }}>
                    {layer.name}
                  </span>
            ...
            {preIpoEntries.map((item) => (
              <div key={`${item.ticker}-${item.name}`} style={{ padding: "12px 0", borderBottom: "1px solid rgba(28,28,48,0.4)" }}>
                ...
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--text-dim)" }}>
                  {item.layer} · {item.rationale || item.trigger || item.entry}
                </div>
              </div>
            ))}
          </div>
